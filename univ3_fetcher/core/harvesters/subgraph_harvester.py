...
# Get config path from environment or use default relative to repo root
_config_path_env = os.environ.get("DATA_FETCH_CONFIG_PATH")
if _config_path_env:
    CONFIG_PATH = _config_path_env
else:
    # Try to find repo root (3 levels up from this file)
    _repo_root = Path(__file__).resolve().parent.parent.parent.parent
    CONFIG_PATH = str(_repo_root / "data_fetch_config.yml")
...
# Module-level variables (initialized when script runs)
CFG: Optional[Dict[str, Any]] = None
GRAPH_URL: Optional[str] = None
JSON_RPC_URLS: Optional[List[str]] = None
POOL_ADDR: Optional[str] = None
START_TS: Optional[int] = None
END_TS: Optional[int] = None
PAGE_SIZE: Optional[int] = None
CHUNK_EVENTS: Optional[int] = None
BATCH_RECEIPT_SIZE: Optional[int] = None
CHECKPOINT_PATH: Optional[str] = None
OUT_DIR: Optional[str] = None
SKIP_GAS_DATA: Optional[bool] = None
COMPUTE_PRICE: Optional[bool] = None
...
def run_harvester():
    """Run the subgraph harvester with loaded configuration."""
    global CFG, GRAPH_URL, JSON_RPC_URLS, POOL_ADDR, START_TS, END_TS, PAGE_SIZE
    global CHUNK_EVENTS, BATCH_RECEIPT_SIZE, CHECKPOINT_PATH, OUT_DIR, SKIP_GAS_DATA, COMPUTE_PRICE, w3
    
    CFG = load_config(CONFIG_PATH)
    
    GRAPH_URL = CFG["graph_url"]
    JSON_RPC_URLS = CFG["json_rpc_urls"]
    POOL_ADDR = CFG["pool_addr"]
    START_TS = CFG["start_ts"]
    END_TS = CFG["end_ts"]
    PAGE_SIZE = CFG["subgraph_page_size"]
    CHUNK_EVENTS = CFG["chunk_events"]
    BATCH_RECEIPT_SIZE = CFG["batch_receipt_size"]
    CHECKPOINT_PATH = CFG["checkpoint_path"]
    OUT_DIR = CFG["out_dir"].format(pool=POOL_ADDR)
    SKIP_GAS_DATA = CFG["skip_gas_data"]
    COMPUTE_PRICE = CFG["compute_price_column"]
    ...
    def flush_chunk():
        nonlocal buf_rows, cur_L, cur_sqrt, cur_tick, events_total, chunk_first_block, chunk_last_block, last_window_block
